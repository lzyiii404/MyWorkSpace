...
typedef struct {
    ...
    XLed_state_uio_map maps[ MAX_UIO_MAPS ];
} XLed_state_uio_info;

/***************** Variable Definitions **************************************/
static XLed_state_uio_info uio_info;
...
static int uio_info_read_name(XLed_state_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLed_state_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLed_state_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLed_state_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLed_state_Initialize(XLed_state *InstancePtr, const char* InstanceName) {
	XLed_state_uio_info *InfoPtr = &uio_info;
