...
const char codeLib[] = "Q ";
const char *imgFileName = "C:\\Users\\80944\\Desktop\\get\\1.jpg";
const char *outFileName = "C:\\Users\\80944\\Desktop\\get\\test.txt";

int main() {
    cout << strlen(codeLib) << " : " << codeLib << endl;
    Mat srcImg = imread(imgFileName);
    srcImg = srcImg(Rect(50, 145, 390, 50));
    Mat grayImg;
    ...
    for(int y = 1; y < binImg.rows; y += 3) {
        for (int x = 1; x < binImg.cols; x+=3) {
            int grayVal = (int)binImg.at<uchar>(y, x);
//            cout << grayVal << endl;
            int index = 1.0 / 255.0 * grayVal;
            str += (index ? "X" : " ");
        }
        str += "\n";
    }
