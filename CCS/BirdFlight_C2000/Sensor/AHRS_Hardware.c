...
//    LSM303_Initial();
//    DELAY_US(500*100);
    MPU6500_initialize(); //mpu6500瀵勫瓨鍣ㄩ厤缃�
    DELAY_US(500*10);
    MPU6500_Init_Offset();
    DELAY_US(500*10);
    Set_Cutoff_Frequency(500.0f, 20.0f,&Accel_Parameter);//鍔犺宸寸壒娌冩柉鍙傛暟鍒濆鍖�
    DELAY_US(500*10);
    Set_Cutoff_Frequency(500.0f, 41.0f,&Gyro_Parameter);//闄�铻轰华宸寸壒娌冩柉鍙傛暟鍒濆鍖�
    DELAY_US(500*10);
}
...
void IMU_getValues(float * values)
{
    ...
    GyroData[0] = ((float) accgyroval[3]) / 16.384f  ;
    GyroData[1] = ((float) accgyroval[4]) / 16.384f  ;
    GyroData[2] = ((float) accgyroval[5]) / 16.384f  ;
