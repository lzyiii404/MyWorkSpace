...
static  u8  s_arrSendBuf[UART_BUF_SIZE];     //发送串口Buf
static  u8  s_arrRecBuf[UART_BUF_SIZE];      //接收串口Buf
...
static  void  ConfigUART(u32 bound)
{
  ...
  DEBUG_USART_APBxClkCmd(DEBUG_USART_CLK, ENABLE);    //开启USART的时钟
  DEBUG_USART_APBxClkCmd(DEBUG_USART_GPIO_CLK, ENABLE);     //开启USART的GPIO时钟

  
  //配置USART.Tx的GPIO
  GPIO_InitStructure.GPIO_Pin   = DEBUG_USART_TX_GPIO_PIN;               //设置USART.TX的引脚
  ...
  GPIO_Init(DEBUG_USART_TX_GPIO_PORT, &GPIO_InitStructure);                    //根据参数初始化USART.TX的GPIO端口
  
  //配置USART.Rx的GPIO
  GPIO_InitStructure.GPIO_Pin   = DEBUG_USART_RX_GPIO_PIN;              //设置USART.RX的引脚
  ...
  GPIO_Init(DEBUG_USART_RX_GPIO_PORT, &GPIO_InitStructure);                    //根据参数初始化USART.RX的GPIO端口
  ...
  USART_Init(DEBUG_USARTx, &USART_InitStructure);                 //根据参数初始化USART

  //配置USART的NVIC
  NVIC_InitStructure.NVIC_IRQChannel = DEBUG_USART_IRQ;         //开启USART的中断
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1; //抢占优先级，屏蔽即默认值
  NVIC_InitStructure.NVIC_IRQChannelSubPriority        = 2; //子优先级，屏蔽即默认值
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;           //IRQ通道使能
  NVIC_Init(&NVIC_InitStructure);                           //根据参数初始化USART的NVIC寄存器

  //打开串口使能开关
  USART_ITConfig(DEBUG_USARTx, USART_IT_RXNE, ENABLE);  //接收中断使能
  USART_ITConfig(DEBUG_USARTx, USART_IT_TXE,  ENABLE);  //发送中断使能
  USART_Cmd(DEBUG_USARTx, ENABLE);                      //使能USART
...
/*********************************************************************************************************
* 函数名称: DEBUG_USARTx_IRQHandler
* 函数功能: DEBUG_USARTx中断服务函数 
* 输入参数: void
* 输出参数: void
* 返 回 值: void
* 创建日期: 2018年01月01日
...
void USART2_IRQHandler(void)                
{
  ...
  {
    NVIC_ClearPendingIRQ(DEBUG_USART_IRQ);  //清除接收中断
    uData = USART_ReceiveData(DEBUG_USARTx);  //接收串口传来的数据，并存到uData中    
    WriteReceiveBuf(uData); //当接收到数据时，将接收到的数据写入到接收缓冲区中
  }
  if(USART_GetFlagStatus(DEBUG_USARTx, USART_FLAG_ORE) == SET)  //如果USART溢出错误标志位为“1”
  {
    USART_ClearFlag(DEBUG_USARTx, USART_FLAG_ORE);  //清除USART的溢出错误标志
    USART_ReceiveData(DEBUG_USARTx);  //从USART接收数据 
  }
  
  if(USART_GetITStatus(DEBUG_USARTx, USART_IT_TXE) != RESET)  //发送中断
  {
    USART_ClearITPendingBit(DEBUG_USARTx, USART_IT_TXE);  //清除USART的发送中断标志位
    NVIC_ClearPendingIRQ(DEBUG_USART_IRQ);  //清除发送中断
    
    ReadSendBuf(&uData);  //读取发送缓冲区的数据到uData
    
    USART_SendData(DEBUG_USARTx, uData);  //串口发送数据uData
                                                                                           
    if(QueueEmpty(&s_structUARTSendCirQue)) //当发送缓冲区数据长度为0
    {                                                               
      s_iUARTTxSts = UART_STATE_OFF;  //USART.TX状态标志设置为关闭       
      USART_ITConfig(DEBUG_USARTx, USART_IT_TXE, DISABLE);  //关闭串口发送中断
    }
  }
}  
