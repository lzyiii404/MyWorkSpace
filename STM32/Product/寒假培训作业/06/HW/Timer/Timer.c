...
static  void  ConfigTimer4(u16 arr, u16 psc);     //配置定时器TIM2的参数
...
static  void ConfigTimer4(u16 arr, u16 psc)
{
  ...
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);          //使能定时器TIM2的时钟
  ...
  TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;       //设置时钟分割:TDTS = Tck_tim
  TIM_TimeBaseStructure.TIM_CounterMode   = TIM_CounterMode_Up; //设置定时器TIM2为向上计数模式
  TIM_TimeBaseInit(TIM4, &TIM_TimeBaseStructure);               //根据指定的参数初始化TIM2的时间基数单位
 
  TIM_ITConfig(TIM4, TIM_IT_Update, ENABLE);                    //使能指定的定时器TIM2中断，并且允许更新中断

  //中断优先级NVIC设置
  NVIC_InitStructure.NVIC_IRQChannel = TIM4_IRQn;               //设置定时器TIM2的中断通道
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;     //设置先占优先级为0级
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 3;            //设置从优先级为3级
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;               //使能中断NVIC的中断通道
  NVIC_Init(&NVIC_InitStructure);                               //根据参数初始化中断NVIC的寄存器
...
void TIM4_IRQHandler(void)  
{
  ...
  if(TIM_GetITStatus(TIM4, TIM_IT_Update) == SET)    //检查到TIM10更新中断的发生
  {
    TIM_ClearITPendingBit(TIM4, TIM_FLAG_Update);	   //清除TIM10更新中断标志
  }
  ...
  if(s_iCnt10 >= 10)                                   //10ms计数器的计数值大于或等于10
  {
    ...
    s_i10msFlag = TRUE;                               //将10ms标志设置为TRUE
  }
}
...
* 注    意: 
*********************************************************************************************************/
void  Clr10msFlag(void)
{
  s_i10msFlag = FALSE;     //将10ms标志设置为FALSE 
}
