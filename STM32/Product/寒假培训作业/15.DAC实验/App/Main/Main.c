...
static  void  Proc2msTask(void)
{
  ...
  {     
    ...
    if(s_iCnt4 >= 4)                  //达到8ms
    {
      ...
      if(ScanKeyOne(KEY_NAME_KEY2, ProcKeyUpKey2, ProcKeyDownKey2))
        mul += 0.1;
      if(ScanKeyOne(KEY_NAME_KEY3, ProcKeyUpKey3, ProcKeyDownKey3))
        mul -= 0.1;

      if(ReadADCBuf(&adcData))        //从缓存队列中取出1个数据
      {
        waveData = (u16)((float)((adcData * 127) / 4095) * mul);    //计算获取点的位置
